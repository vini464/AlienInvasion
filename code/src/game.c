...
int main(void) {
  ...
  for (i=0; i<2; i++) {
    ...
    TANK.sprite[i].pos_x = CENTER + 20 * (i - 1) - i;
    ...
    SHIP.sprite[i].pos_x = CENTER + 20 * (i - 1) -i ;
  ...
  for (i=0; i<MAX_SHOTS; i++) {
    ...
    SHIP.shots[i].mem_offset = 23; // TODO: alterar
    SHIP.shots[i].pos_y = 0;
    SHIP.shots[i].pos_x = 0;

    TANK.shots[i].act = 0;
    TANK.shots[i].reg_id = i + 7;
    TANK.shots[i].mem_offset = 9; // TODO: alterar
...
void * accelThread(void * arg) {
  ...
  while (!(*stop)) {
    get_direction(&direction);
      ...
      TANK.sprite[0].pos_x += direction.x/20; // só vai ser movimentado no eixo x
      TANK.sprite[1].pos_x += direction.x/20; // só vai ser movimentado no eixo x
...
void * mouseThread(void * arg) {
  ...
  while (!(*stop)) {
    ...
    for (i=0; i<2; i++) {
      ...
      if (SHIP.sprite[i].pos_y > BOTTOM/2 - 20) // TODO: trocar 20 por tamanho do sprite
        SHIP.sprite[i].pos_y = BOTTOM/2-20;
  ...
  while (!(*stop)) {
    for (i=0; i<MAX_SHOTS; i++) {
      if (TANK.active_shots[i]){
        ...
        if (checkCollision(&TANK.shots[i], &SHIP.sprite[0]) || checkCollision(&TANK.shots[i], &SHIP.sprite[1]) || TANK.shots[i].pos_y < TOP){
          TANK.active_shots[i] = 0;
          TANK.avaiable_shots++;
          printf("adicionou i=%d\n", i);
          TANK.shots[i].act = 0;
      ...
      if (SHIP.active_shots[i]){
        ...
        if (checkCollision(&SHIP.shots[i], &TANK.sprite[0]) || checkCollision(&SHIP.shots[i], &TANK.sprite[1]) || SHIP.shots[i].pos_y > BOTTOM - 20){ // TODO: alterar constante 20 para um valor variável dentro da sprite, fazer isso tambem com TOP 
